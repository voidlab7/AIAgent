...
### 第二步：确保 sshpass 可用（关键步骤）

sshpass 是密码认证 SCP/SSH 的必要工具。**必须按以下优先级检查**：
...
1. **sshpass 检测优先级** — 先检查 `~/bin/sshpass` 是否存在（`ls ~/bin/sshpass`），存在则直接用 `~/bin/sshpass`；不存在再检查 `which sshpass`；都没有则从源码自动编译安装到 `~/bin/`
...
7. **密码中包含特殊字符** — 密码用单引号包裹，避免 shell 解析逗号等字符
